...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        mball->draw(model2);
        terrain->draw(model3, cam.pos_);
        quad->draw(model, cam.pos_);
        skyBox->draw();

        string x = to_string(cam.pos_.x).substr(0, 4);
        string y = to_string(cam.pos_.y).substr(0, 4);
        string z = to_string(cam.pos_.z).substr(0, 4);
